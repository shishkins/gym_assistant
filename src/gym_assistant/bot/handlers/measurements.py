...
async def _save_weight(message: Message, session: AsyncSession, user: User, value: Decimal) -> None:
    ...
    shown = render.format_weight(value)
    ...
    if delta > 0:
        wording = ru.WEIGHT_DELTA_UP.format(value=render.format_weight(delta))
    elif delta < 0:
        wording = ru.WEIGHT_DELTA_DOWN.format(value=render.format_weight(-delta))
    else:
    ...
    if last is not None and last.weight_kg is not None:
        await message.answer(
            ru.WEIGHT_PROMPT_WITH_LAST.format(
                last=render.format_weight(last.weight_kg),
...
async def photo_received(
    ...
    if weight is not None:
        await message.answer(ru.PHOTO_SAVED_WITH_WEIGHT.format(weight=render.format_weight(weight)))
    else:
    ...
    for measurement in reversed(photos):
        ...
        caption = (
            ru.PHOTO_CAPTION_WITH_WEIGHT.format(
                date=when, weight=render.format_weight(measurement.weight_kg)
            )
