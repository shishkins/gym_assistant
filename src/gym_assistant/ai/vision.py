...
Если указана калорийность блюда — сделай это блюдо ОДНОЙ позицией с этими
числами. Название возьми со скриншота, граммовку прикинь по фото, а состав
на 100 г посчитай из указанных калорий и Б/Ж/У.

Указанная калорийность покрывает блюдо ЦЕЛИКОМ, со всем, что в него входит:
заправка, соус, гарнир, добавки — даже если они поданы в отдельной баночке
рядом. НЕ добавляй их отдельными позициями: это двойной счёт. Если в
названии блюда есть «with ... dressing», «с соусом», «со сметаной» — соус
уже внутри.

Отдельной позицией считай только то, что явно не входит в это блюдо:
напиток, купленный сам по себе, второе блюдо, подарок к заказу.
